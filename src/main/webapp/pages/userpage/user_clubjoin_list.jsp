...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：社团及活动管理&nbsp;&nbsp;>&nbsp;&nbsp;社团申请记录</td>
  </tr>
...
<%--  </tr>--%>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">社长名</th>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="center" valign="middle" class="borderright borderbottom">${clubjoin.clubName }</td>
          <td align="center" valign="middle" class="borderright borderbottom">${clubjoin.clubmanagerName }</td>
          ...
          <td align="center" valign="middle" class="borderbottom">
              <c:choose>
                  <c:when test="${clubjoin.clubjoinState == 1 }">
                      <a href="DeleteOneClubJoin?username=${clubjoin.userjoinName}&clubname=${clubjoin.clubName}&startnum=1"  target="mainFrame" onFocus="this.blur()" class="add" onclick="return confirm('确定取消吗？')">取消</a>
                  </c:when>
  ...
  <tr>
      <td align="left" valign="top" class="fenye">共有${clubjoinpage.total} 条记录，当前第${clubjoinpage.pageNum}页
          &nbsp; &nbsp; &nbsp;
          <a href="SelectOneClubJoin?startnum=1&username=${sessionScope.user.userName}">首页</a>
          <c:if test="${clubjoinpage.pageNum>1}" ><a href="SelectOneClubJoin?startnum=${clubjoinpage.prePage}&username=${sessionScope.user.userName}">上一页</a></c:if>
          <c:if test="${clubjoinpage.pageNum==1}" ><a href="SelectOneClubJoin?startnum=1&username=${sessionScope.user.userName}">上一页</a></c:if>
          <c:if test="${clubjoinpage.pageNum<clubjoinpage.pages}" ><a href="SelectOneClubJoin?startnum=${clubjoinpage.nextPage}&username=${sessionScope.user.userName}">下一页</a></c:if>
          <c:if test="${clubjoinpage.pageNum==clubjoinpage.pages}" ><a href="SelectOneClubJoin?startnum=${clubjoinpage.pages}&username=${sessionScope.user.userName}">下一页</a></c:if>
          <a href="SelectOneClubJoin?startnum=${clubjoinpage.pages}&username=${sessionScope.user.userName}">尾页</a>
    ...
    <tr>   <td><div align="center" style="color: #8A0B00">${requestScope.msg}</div></td></tr>
      ...
      <td width="90%" style="color: #8A0B00" align="center" valign="middle">
          <form method="post" action="SelectOneClubJoin?startnum=1&username=${sessionScope.user.userName}">
